...
def generate_clip2d_SAM_SortBboxBackground_Interpolation(flag, image, output_path):
    ...
    for i in range(masks_number):
        ...
        #bbox
        bbox = masks[i]["bbox"]
        if (i == 0) or (bbox[3] + 1 == image_h and bbox[2] + 1 == image_w):
            continue
...
if __name__ == "__main__":
    root = "/Users/jfgvl1187/Desktop/segment-anything/data/" 
    #root = "/users/aren10/data/datasets/"
    if os.path.exists(root + "cabinet03_random/.DS_Store"):
        os.remove(root + "cabinet03_random/.DS_Store")
    input_path = root + "cabinet03_random/"
    image_dic1 = os.listdir(input_path)
    for image_dic2 in image_dic1:
        if image_dic2[0:3] == "cam":
            print(image_dic2)
            new_image_name = "r_" + str(int(image_dic2[-2:])) + ".png"
            for image_path in os.listdir(input_path + image_dic2):
                if image_path[-4:] == ".png":
                    os.rename(input_path + image_dic2 + "/" + image_path, input_path + new_image_name)
    input_path = root + "cabinet03_random/"
    method = "SAM_SortBboxBackground_Interpolation"
    output_path = root + "cabinet03_random/"
    test_image_num = "r_40"
    text = "cabinet"
    thr = 0.9
